...
% Retitled Aug 2026: was "A Three-Module Test Cell". Two of the three modules are
% the same TX/TL reactions in different formalisms (see the SSA node below), which
% is a control that makes boundary error attributable -- something a test rig has
% and a cell would not. "Cell" invited "why does a cell need two descriptions of
% its own transcription?"; "harness" does not. Count is unchanged and still true:
% three module objects, two distinct biological subsystems.
\begin{frame}{A Three-Module Test Harness}
	...
	\begin{columns}[T,onlytextwidth]
		\begin{column}{0.38\textwidth}
			...
			{\tiny ODE metabolism, continuous kinetics}\\[1pt]
			...
			% NOT "bursty": the cell in these figures uses StochasticGeneExpression,
			% which has no promoter switching (no k_on/k_off) -- it is the txl
			% reactions as a jump process. BurstyGeneExpression is a different cell
			% (the step4 one), whose figures are not in this deck.
			{\tiny Gillespie SSA, discrete molecule counts}\\[1pt]
			\begin{tikzpicture}[scale=0.34]
				\draw[->] (0,0) -- (4.6,0) node[right, font=\tiny] {$t$};
				\draw[->] (0,0) -- (0,2.3);
		...
		\begin{column}{0.60\textwidth}
			...
			\begin{tikzpicture}[
				...
				\node[mod, fill=blue!12] (txl) at (-1.15,0) {\textbf{TX/TL}\\(ODE)};
				\node[mod, fill=blue!12] (met) at (1.15,0)  {\textbf{Metab.}\\(ODE)};
				...
				% Labelled TX/TL, not "Gene expr.": StochasticGeneExpression is the
				% SAME four reactions as TranscriptionTranslation above (same states
				% [mRNA, protein], same k_tx/k_tl/gamma_mRNA/gamma_protein, registered
				% under the same names -- which is exactly why all four deduplicate).
				% The ODE block is its mean-field limit. Two different names hid the
				% one thing this cell exists to demonstrate: the same biology modelled
				% twice, so the two posteriors can be compared. Identical bold labels
				% plus differing formalism tags make the duplication self-evident.
				\node[mod, fill=orange!12] (ssa) at (-1.15,-2.25) {\textbf{TX/TL}\\(SSA)};
				...
				\node[font=\tiny\itshape, text=gray!70, anchor=west, align=left,
					text width=1.9cm] at (-0.15,-2.25)
					{same reactions,\\ two formalisms};

				\draw[->, very thick, red!60!black] (diffblock.south -| ssa) -- (simblock.north);
				% Named rather than enumerated: the individual rate symbols carry no
				% meaning without the equations, which this talk never shows. The count
				% is the part that informs -- 4 of the 8 are shared. Numbers verified
				% against export_results.jl's name lists and the exported CSVs, not
				% inferred from the plots.
				% Explicit breaks: at this width LaTeX hyphenates "parameters".
				\node[font=\tiny\itshape, text=red!60!black, anchor=west, align=left,
					text width=1.95cm] at (-0.25,-1.15)
					{conditioned on the\\ posterior over\\
					$\boldsymbol{\theta}_\mathrm{shared}$};
				...
				\node[draw, thick, rounded corners=3pt, fill=green!15, font=\tiny,
					...
					(data) at (-3.6,-1.15) {observed\\data $\mathbf{y}$};
				...
				\node[font=\tiny\bfseries, text=gray!70, anchor=south]
					at (cell.north) {Three-module test harness, $\boldsymbol{\theta}$};
			\end{tikzpicture}
		\end{column}
	\end{columns}
	...
	\scriptsize
	\begin{itemize}\itemsep1pt
		\item $\boldsymbol{\theta}$: \textbf{8} unique parameters after
		deduplication.
		$\boldsymbol{\theta}_\mathrm{shared} \subset \boldsymbol{\theta}$
		\item A \textbf{synthetic twin}: $\mathbf{y}$ is generated from the model
		being fitted, so ground truth is known.
		\item \textbf{Goal:} the joint posterior
		$p(\boldsymbol{\theta} \mid \mathbf{y})$ --- all 8, conditioned on all the
		data at once.
	\end{itemize}
\end{frame}
...
% ----------------------------------------------------------------
% Results. The SSA figure leads on purpose: it is the only figure in the deck
% that shows information crossing a module boundary and paying for itself
% (unconditioned blue is broad and multimodal and misses truth; conditioned
% green is tight and centred on it). The 9-parameter corner is the supporting
% methods check, and it also pays off the identifiability bullet promised back
% on "What Becomes Possible (Science)" -- the tilted ellipses are that claim
% measured rather than asserted.
%
% Figures only, no prose: at 15 minutes this slide is spoken, not read, and
% dropping the text is what lets both plots run to ~5.8cm. The headings label
% each panel with the symbol slide 10 defined -- the 4-parameter plot is
% theta_shared, the 8-parameter one is all of theta.
%
% "theta" on the right panel is exact, not an approximation: the SSA module here
% is StochasticGeneExpression, whose four parameters are all shared with the
% differentiable block, so the whole system after deduplication is the same 8
% the differentiable block carries. Both panel counts were verified against
% export_results.jl's name lists and the exported CSVs (ode_chain.csv has 8
% columns; cond/uncond_particles.csv have 4).
%
% Aug 2026: both figures replaced from PR #36 (branch talk-figure-legible-fonts),
% which enlarged the fonts for projector legibility -- typography only, no change
% to the palette or the statistics. The right panel is now step3_corner_reduced,
% a 4-of-8 marginal cut of the full corner: the 8x8 grid at 5.8cm tall put the
% axis labels below reading size from the back of a room. The chain is still the
% joint over all 8 and the caption says so -- what is cut is the display, not the
% inference. The full 8x8 (step3_corner.png, also refreshed) is still in figures/
% if the whole grid is ever wanted for Q&A.
%
% CAUTION: a second, different three-module cell exists in the repo
% (BurstyGeneExpression instead of StochasticGeneExpression), for which the
% counts are 8 differentiable / 6 simulation / 11 total with only 3 shared. Its
% figure is step4_corner_talk_shared.png. Do not mix that figure into this slide
% or reuse these numbers for it -- both are right, for different cells.
%
% NOTE: the deck no longer states anywhere -- main line or backup -- that the
...
% seeds at a nominal 95%, so single-pass is the default). Slide 10 promises the
% joint posterior as the goal and nothing later qualifies it. If that caveat is
% wanted back, the Open Questions slide is where it belongs, phrased as the
% coupling question; step4_calibration_curve.png is still in figures/.
\begin{frame}{Results: Conditioning Across the Boundary Works}
	\vspace{-1mm}
	% \par inside each caption group: without it the group closes before the
	% paragraph ends, so \tiny's \baselineskip is discarded and the captions set
	% on the outer \normalsize leading.
	\begin{columns}[T,onlytextwidth]
		\begin{column}{0.49\textwidth}
			\centering
			{\small $\boldsymbol{\theta}_\mathrm{shared}$ --- simulation block}\\[2pt]
			\includegraphics[height=5.8cm]{figures/step3_ssa_posteriors.png}\\[2pt]
			{\tiny\itshape Blue: SSA inferred alone. Green: conditioned on the
			differentiable block. Orange: truth.\par}
		\end{column}
		\begin{column}{0.49\textwidth}
			\centering
			{\small $\boldsymbol{\theta}$ --- differentiable block}\\[2pt]
			\includegraphics[height=5.8cm]{figures/step3_corner_reduced.png}\\[2pt]
			{\tiny\itshape Joint NUTS over all 8; 4 shown. Tilted ellipses: the
			data constrains \emph{combinations} of rates.\par}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c,onlytextwidth]
		\begin{column}{0.68\textwidth}
			...
			\begin{enumerate}\itemsep3pt
				...
				\item \textbf{Toy pilot works.} A three-module test harness, ODE and SSA in
				one graph: conditioning across the module boundary sharpens the
				posterior onto truth. On synthetic data, so far.
