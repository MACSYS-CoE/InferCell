...
%
% Reduced variants exist but are NOT used below: --subset shared (3x3),
% --subset matched (4x4, same shape as the NUTS corner) and --mode ode --box
% (the NUTS corner with the mirror-image shared box). Swap them in here if the
% 6x6 does not read from the back of the room.
%
% isab_ssa_alone_matched is built with --no-legend so it pairs with the NUTS
% corner, which has no legend either. If this frame adopts that pair, add a
% figTruth chip to the caption below -- the legend was what named the magenta
% truth lines, and nothing else on the frame does.
%
