...
%
% The closing bullet is a QUESTION, not a goal. That is deliberate and it is the
% fix for the transition that always read badly in practice: the results slide
% shows a conditioned-vs-unconditioned contrast, and the old "Goal: the joint
% posterior" bullet never set that contrast up, so the comparison arrived
% unannounced. Now the next slide answers the question this one asks. If either
% slide is edited, keep that pairing.
...
\begin{frame}{A Minimal Cell, in Three Modules}
	...
	\begin{columns}[T,onlytextwidth]
		...
		\begin{column}{0.60\textwidth}
			...
			\begin{tikzpicture}[
				...
				\node[mod, fill=blue!12] (txl) at (-1.15,0) {\textbf{Enzyme gene}\\(ODE)};
				...
				\node[mod, fill=orange!12] (ssa) at (-1.15,-2.25) {\textbf{Regulator gene}\\(SSA)};
				...
				% same machinery as the enzyme gene, so k_tl, gamma_mRNA and
				% gamma_protein are literally the same three numbers -- that is the
				% whole reason anything crosses this arrow. Count verified against
				% the parameter lists in the two model constructors: 3 of 11.
				% Explicit breaks: at this width LaTeX hyphenates "ribosomes".
				\node[font=\tiny\itshape, text=red!60!black, anchor=west, align=left,
					text width=2.0cm] at (-0.25,-1.15)
					{same ribosomes,\\ same degradation:\\
					\textbf{3 of 11} shared};
	...
	\scriptsize
	\begin{itemize}\itemsep1pt
		\item $\boldsymbol{\theta}$: \textbf{11} unique parameters after
		deduplication; \textbf{3} shared between the two expression modules.
		\item A \textbf{synthetic twin}: $\mathbf{y}$ is generated from the model
		being fitted, so ground truth is known.
		% The question the next slide answers. Ask it out loud, then advance.
		\item \textbf{The question:} the bulk data never sees the regulator gene.
		Can it still tell us anything about single-molecule bursting?
	\end{itemize}
\end{frame}
...
\begin{frame}{Results: Yes --- Where the Modules Share Machinery}
	\vspace{-1mm}
	\begin{columns}[c,onlytextwidth]
		\begin{column}{0.60\textwidth}
			\centering
		...
		\begin{column}{0.38\textwidth}
			\footnotesize
			{\color{colODELine}$\blacksquare$}~\textbf{Alone} --- ABC-SMC on the
			regulator's own data.

			\smallskip
			{\color{green!45!black}$\blacksquare$}~\textbf{Conditioned} --- shared
			parameters carry the differentiable block's posterior.
			...
			\smallskip
			$\rightarrow$~The other 3 govern \textbf{promoter switching} and
			barely move --- the bulk data never sees them, and mean trajectories
			cannot separate them anyway.

			\medskip
			{\scriptsize\itshape Information crossed the boundary --- and stopped
			where the coupling stopped.\par}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}[noframenumbering]{Is the Differentiable Half Identifiable?}
	\begin{columns}[c,onlytextwidth]
		...
		\begin{column}{0.40\textwidth}
			\small
			Joint NUTS over the differentiable block: metabolism and the enzyme
			gene, all \textbf{8} parameters at once. 4 rates shown.

			\medskip
			\begin{itemize}\itemsep3pt
				\item Orange lines mark truth.
