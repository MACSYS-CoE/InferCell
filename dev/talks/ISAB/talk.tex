...
\begin{frame}{But the Uncertainty Compounds}
	...
	\begin{column}{0.27\textwidth}
		...
		\begin{tikzpicture}[
			...
			% Names what both boxes are already showing. The provenance lines
			% ("measured in E. coli", "extrapolated from M. pneumoniae") were
			% doing this work silently; the ISAB review asked for the claim to
			% be made out loud, and this is the cheapest place to make it.
		\end{tikzpicture}
	\end{column}
...
% VERBAL: When MC4D predicts a 105-minute doubling time, we cannot distinguish
% whether (a) the prediction is robust across plausible parameters, or
% (b) it is fragile and depends on specific choices. That distinction is the
% pincer, which is now a BACKUP slide ("Can We Build One?"). Land the
% distinction here in one sentence; pull the slide up only if the "but it
% works, their doubling time matches" objection actually arrives.
%
% (Was 110 throughout the deck. Changed Aug 2026 to 105, the figure the
% project notes carry for syn3A, so the pincer slide, slide 8's illustration
% and this note all quote one number. If 110 is right they move back together.)
...
\begin{frame}{Inverting the Question}
	...
	% The core claim, stated rather than implied. Added Aug 2026 after the ISAB
	% review: the deck showed the inversion but never conceded that it costs
	% more, and a concession is what makes the claim land as a judgement rather
	% than as advocacy. Say it as a concession -- "yes, this is the harder
	% way" -- then take the second half without hedging.
	\vfill
	\centering
	% Must stay one line: wrapped, it orphans the last word and the concession
	% loses its snap.
	\vspace{1mm}
\end{frame}
	...
	\begin{columns}[c,onlytextwidth]
		\begin{column}{0.55\textwidth}
			...
			\begin{itemize}\itemsep6pt
				...
				\item \textbf{No differentiability} --- dies at the
				boundary.
	...
	\begin{columns}[T]
		...
		\begin{column}{0.49\textwidth}
			\begin{itemize}\itemsep3pt\parskip0pt
				...
				% 105, not 110: same number as the pincer backup slide, whose
				% wide horn is this same 60--200 range. Deliberate, so the two
				% agree if that slide gets pulled up.
				{\scriptsize\itshape ``Is doubling time 105\,$\pm$\,5\,min, or 60--200?''}
...
% Parameter ORDER in that figure is load-bearing for the FULL 6x6 exports (now
% backup only, see SHARED-ONLY below): the three unshared parameters (k_on,
% k_off, k_tx_burst) come first and the three shared ones (k_tl, gamma_mRNA,
% gamma_protein) last, so the tightening is confined to the bottom-right block
% and the dashed box sits exactly on the coupling. If the export order changes,
% the plot script warns and drops the box rather than drawing a wrong one --
% check the log before trusting a regenerated 6x6 figure.
%
...
%
% Consequence: slides 11 and 12 are a build in content, not in pixels -- same
% figure, larger, with the green added.
%
% MATCHED, Aug 2026. Slides 11 and 12 use the reduced 4x4 exports, not the 6x6:
%   isab_ode_corner_boxed.png        (--mode ode --box)
%   isab_ssa_alone_matched.png       (--mode alone --subset matched)
%   isab_ssa_posteriors_matched.png  (--mode ssa   --subset matched)
% A 6x6 corner is fine on paper and unreadable from the back of a lecture
% theatre. MATCHED_SUBSET mirrors ODE_SUBSET -- one transcription rate plus the
% three shared parameters, k_tx_burst standing in for k_tx -- so the two corners
% on slide 11 have the same shape with the same parameters in the same places,
% and --mode ode --box puts the mirror-image dashed box on the NUTS side. Each
% side names the same three parameters from its own vantage; the panels are
% comparable rather than merely the same size.
%
% What is NOT on these figures: k_on and k_off. Everything the deck claims about
% them is spoken, backed by the numbers below -- they are near-uncorrelated with
% the pinned three and genuinely do not move. Keep the slide-12 bullet honest
% about their being unshown. k_tx_burst IS shown, outside the box, and visibly
% tightens, which is the "reached as far as the coupling carried it" beat made
% visual rather than asserted. The full 6x6 versions (isab_ssa_alone.png,
% isab_ssa_posteriors.png) are still in figures/ and are what to pull up if
% anyone asks to see the promoter rates.
%
% --subset shared (3x3, the shared parameters alone) also exists and is a
% further fallback if even 4x4 does not read. It carries no box -- every
% parameter on it is a shared one -- so the right-hand column would have to
% carry that fact in words instead.
%
...
%
% Cutting a figure down cannot move an axis -- each parameter's range is
% computed from BOTH posteriors independently of which panels are kept -- so the
% 4x4s and the 6x6s can be shown in sequence without anything shifting.
%
% Regenerate ALL FOUR together after any palette change -- burst draws from the
% same constants and backup slide 17 goes stale with the rest. Drop --box and
% --subset to refresh the full-size backups too.
%
% isab_ssa_alone_matched is built with --no-legend so it pairs with the NUTS
% corner, which has no legend either. Easy to lose on a rerun -- the flag is on
% the regeneration line above; without it the frame gets one keyed panel beside
% an unkeyed one, which reads as a difference between the two quantities rather
% than a difference in annotation.
%
% Because neither corner carries a legend now, NOTHING on the frame named the
% magenta truth lines -- the figTruth chip above the closing sentence is what
% replaces that. Do not drop it while these two exports are in use. Slide 12
% keeps its legend (orange against green genuinely needs one) and names truth
% there itself.
%
...
%
% Equal columns and equal heights, now that both corners are 4x4 over the same
% four parameters. That symmetry IS the slide: the only thing that differs
% between the two pictures is how broad the contours are, so it is the only
% thing a viewer can be looking at.
\begin{frame}{One Way: Each Block, Its Own Sampler}
	\vspace{-2mm}
	% [b] rather than [c]: the two exports have slightly different aspect ratios
	% and the caption blocks are what must line up, not the image tops.
	\begin{columns}[b,onlytextwidth]
		\begin{column}{0.48\textwidth}
			\centering
			% _boxed, not the plain corner: the green dashed box marks the same
			% three parameters the SSA corner boxes, so each side names the
			% coupling from its own vantage. Point at one box, then the other.
			\includegraphics[height=5.4cm]{figures/isab_ode_corner_boxed.png}\\[1mm]
			{\scriptsize{\color{figDiff}$\blacksquare$}~\textbf{Differentiable
			block} $\cdot$ NUTS\par}
			% Keep the two columns to the same number of \tiny lines: with [b]
			% alignment an odd line out on one side pushes that corner up and the
			% two captions stop lining up.
			{\tiny all 8 parameters jointly, 4 shown\par}
		\end{column}
		\begin{column}{0.48\textwidth}
			\centering
			...
			\IfFileExists{figures/isab_ssa_alone_matched.png}{%
				\includegraphics[height=5.4cm]{figures/isab_ssa_alone_matched.png}%
			}{%
				\fbox{\begin{minipage}[c][5.2cm][c]{0.90\linewidth}
					...
					-{}-mode alone -{}-subset matched}\\ against the run's particle
					CSVs (\texttt{examples/results}).\par}
				\end{minipage}}%
			}\\[1mm]
			{\scriptsize{\color{figAlone}$\blacksquare$}~\textbf{Regulator}
			$\cdot$ ABC-SMC\par}
			{\tiny all 6 parameters jointly, 4 shown\par}
		\end{column}
	\end{columns}
...
\begin{frame}{Another Way: let information flow between modules}
	...
	\begin{columns}[c,onlytextwidth]
		\begin{column}{0.60\textwidth}
			\centering
			\includegraphics[height=6.4cm]{figures/isab_ssa_posteriors_matched.png}
		\end{column}
		...
		\begin{column}{0.38\textwidth}
			...
			% One bullet, not two: the matched 4x4 keeps k_tx_burst and drops k_on,
			% k_off, so this beat has two halves now, but splitting it into two
			% bullets overran the frame top and bottom. Four beats is the ceiling.
			% k_tx_burst tightens 3.1x despite never being conditioned -- the six
			% are fitted jointly and it is correlated with the pinned parameters
			% (corr +0.36 with gamma_prot, -0.24 with k_tl), which is visible on
			% the figure. k_on and k_off are near-uncorrelated and genuinely do not
			% move; they are off this figure, so say so.
	...
	\begin{columns}[c,onlytextwidth]
		\begin{column}{0.68\textwidth}
			...
			\begin{enumerate}\itemsep3pt
				% Framed as maturation rather than deficiency, to bookend the
				% "can we build one? / is it right?" slide. Same length as the
				% old "take ~1000 parameters from literature and simulate
				% forward" wording, so the item still sets in four lines --
				% check that if it is edited again.
				\item \textbf{The problem.} Whole-cell models can now simulate a
				cell end to end --- from ${\sim}$1000 literature parameters,
				fixed. No uncertainty quantification, no way to learn from data,
				...
				\item \textbf{Toy pilot.} A three-module minimal cell, synthetic data.  Information crosses the module boundary and
				sharpens the posterior onto truth.
				\item \textbf{Next steps} Real datasets to condition
...
% ----------------------------------------------------------------
% THE PINCER. First backup on purpose: it answers the single likeliest
% challenge from this board, so it is the one to be able to reach without
% hunting. Written Aug 2026 after ISAB review feedback that the core claim ---
% this is the harder way, and the better one --- was implied rather than
% stated. The maturation framing it carries is now SPOKEN in the main line
% (the pivot slide's closing alert, Summary item 1, the in-vitro/in-vivo label
% on the compounding slide); this slide is the picture for it if the argument
% actually gets contested.
%
% WHEN TO PULL IT UP: someone says a version of "but it works --- their
% doubling time matches the measurement, what is the problem?". The pincer
% answers without needing to know whether the model was tuned: propagate the
% literature uncertainty and exactly one of two things is true, and both
% defeat the match as evidence. Full argument in docs/explorations/
% 2026-07-22-modularity-misspecification-flip-sharpener.md, "Killer
% Alternative Explanation", item 1.
%
% THE NARROW HORN IS THE ONE THAT GETS PUSHED BACK ON. Do not read the bullet
% aloud -- walk the middle step:
% - The premise is that the INPUTS are wide (2--6x literature ranges, which
%   the compounding slide establishes). A narrow output from wide inputs means
%   the doubling time is insensitive to the rates.
% - If the prediction does not move when theta moves, observing it cannot
%   inform theta. Flat likelihood; the posterior comes back as the prior. It
%   is the identifiability point from "What Becomes Possible", arriving
%   backwards.
% - Concrete version: perturb 200 rate constants by 3x each, stay
%   literature-consistent, still get 105. A model with 200 wrong rates passes
%   the doubling-time test exactly as well as the right one.
% - CONCEDE FIRST. A tight prediction that matches reality IS good, for
%   prediction. The claim is about what the match validates. What it validates
%   is the coarse structure -- stoichiometry, mass balance, ribosome budget --
%   which is the part nobody was worried about; it says nothing about the
%   ~1000 numbers that are.
% - Do NOT say the narrow horn means the model is wrong. It does not.
%
% ALSO WORTH SAYING: narrow is probably not the case we are in. For a
% ~1000-parameter nonlinear model with those input ranges the doubling-time
% distribution is almost certainly wide. The narrow horn exists to close the
% escape route ("well, maybe it is robust"), not because we expect it. Saying
% so makes the slide read as rigour rather than as a gotcha.
%
% The strongest counter is "we do not validate on one scalar --- Karr matched
% hundreds of observables". Answer constructively: then the real tests are the
% observables that are BOTH sensitive to the parameters AND were not used
% during construction. Naming those is itself an inference calculation.
%
% NUMBER TO CONFIRM: ~105 min is the syn3A doubling time carried in the
% project notes (the sharpener above). Slide 8's illustration and the verbal
% note on the compounding slide both used to say 110; all three now say 105,
% so if the 2026 paper's figure is 110 they move back together.
%
% Geometry: coordinates in cm, no scale= key -- TikZ scales positions but not
% node minimum sizes. The two horn panels are deliberately IDENTICAL except
% for the width of the curve: same axis, same magenta measured-value line,
% same caption shape. Width is then the only thing a viewer can be looking at,
% the same design rule the two corner plots on slide 11 follow.
\begin{frame}[noframenumbering]{``Can We Build One?'' is Answered. ``Is It Right?'' is Not.}
	\small
	\textbf{First generation} --- \emph{can a whole cell be simulated at all?}
	Karr 2012, Thornburg 2026: \textbf{yes}.\\
	\textbf{Second generation} --- \emph{are the numbers right, and how would
	we know?}
	\par\smallskip
	{\footnotesize The reflex: \emph{``but it works --- our doubling time
	matches the measurement.''} Propagate the uncertainty and there are only
	two cases:}
	...
	\begin{tikzpicture}[>=Stealth]
		% ===== The reported match =====
		\node[draw, thick, rounded corners=4pt, fill=gray!12, font=\scriptsize,
			align=center, inner sep=4pt] (rep) at (0,2.55)
			{predicted doubling time ${\sim}105$\,min
			\;$=$\; measured ${\sim}105$\,min};

		% ===== The two horns =====
		% cx / sd / amplitude. The amplitudes differ so the two curves enclose
		% roughly the same area: a narrow spike drawn at the wide one's height
		% would read as "more probable" rather than "more certain".
		\foreach \cx/\sd/\amp in {-3.55/0.95/0.62, 3.55/0.26/0.86}{
			\draw[thick, rounded corners=5pt, gray!45]
				(\cx-3.0,-1.62) rectangle (\cx+3.0,1.62);
			% Axis, with the measured value marked BELOW it. Above the axis is
			% where the narrow curve's peak lives, so a label up there sits on
			% the curve in one panel and in clear air in the other -- which
			% breaks the panels' deliberate symmetry.
			\draw[thin, gray!60] (\cx-2.5,0.62) -- (\cx+2.5,0.62);
			\node[font=\tiny, text=gray!70, anchor=north east]
				at (\cx+2.5,0.56) {doubling time};
			% predictive distribution
			\begin{scope}[shift={(\cx,0.62)}]
				\fill[colPost] plot[smooth, domain=-2.4:2.4, samples=97]
					(\x, {\amp*exp(-0.5*(\x/\sd)^2)}) -- (2.4,0) -- (-2.4,0) -- cycle;
				\draw[thick, colPostLine] plot[smooth, domain=-2.4:2.4, samples=97]
					(\x, {\amp*exp(-0.5*(\x/\sd)^2)});
			\end{scope}
			% the measured value, in the deck's truth colour
			\draw[dashed, thick, figTruth] (\cx,0.62) -- (\cx,1.52);
			\node[font=\tiny, text=figTruth, anchor=north]
				at (\cx,0.56) {105\,min, measured};
		}
		% Arrows enter each panel off-centre, on the side facing the box, so
		% they clear the dashed measured-value line at the panel's midline.
		\draw[->, very thick, colWarnLine] (rep.south) -- (-2.05,1.62);
		\draw[->, very thick, colWarnLine] (rep.south) -- ( 2.05,1.62);

		% ===== What each horn costs them =====
		\node[font=\tiny, anchor=north, align=left, text width=5.5cm]
			at (-3.55,0.10)
			{\textbf{Wide.} 105 was one draw. Another equally
			literature-consistent parameter set gives 60, or 200.\\[3pt]
			{\color{colWarnLine}$\Rightarrow$ the match was a
			\textbf{selection}, not a prediction.}};
		% Concedes the good thing FIRST. The earlier version jumped straight
		% to "cannot separate a right model from a wrong one", which reads
		% as a gotcha and invites the obvious objection -- a tight
		% prediction that matches reality is a good outcome. It is, for
		% prediction; the bullet's job is the validation claim, and that
		% needs the middle step (insensitive to the rates => uninformative
		% about the rates) stated rather than assumed.
		\node[font=\tiny, anchor=north, align=left, text width=5.5cm]
			at (3.55,0.10)
			{\textbf{Narrow.} The prediction barely moves when the rates do
			--- so measuring it cannot inform them.\\[3pt]
			{\color{colWarnLine}$\Rightarrow$ tests the
			\textbf{stoichiometry}, not the ${\sim}1000$ numbers.}};
	\end{tikzpicture}

	% One line, and short enough to stay one line: at \small the two-line
	% version pushed the frame overfull and split the punch across a break.
