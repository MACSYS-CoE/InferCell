...
\author{Tom Kimpson}
\institute{Department of Mathematics \& Statistics, \\ University of Melbourne}
\date[2026]{MACSYS UoM Seminar, April 2026}
...
\begin{frame}{Takeaways}
	...
	\begin{enumerate}\itemsep20pt
		...
		\item \alert{InferCell} is a modular, inference-first whole-cell modelling framework in Julia.
		
		\item Interested? Get involved!
	\end{enumerate}
...
\begin{frame}[plain]
	\centering
	\vfill
	{\Large\bfseries The Problem}\\[10pt]
	{\large\color{gray} Why whole-cell models need inference}
	\vfill
\end{frame}
	...
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
			\begin{itemize}\itemsep6pt
				\item Whole-cell models simulate a living cell from its molecular parts
				\item State of the art: Luthey-Schulten Lab's 4D model (MC4D, \textit{Cell} 2026)
				\begin{itemize}\itemsep3pt
					\item 493 genes, 4 coupled solvers
					\item 3D spatial resolution at 10\,nm voxels
					\item GPU-accelerated, full cell cycle
				\end{itemize}
				\item A genuine scientific achievement!
...
% ----------------------------------------------------------------
\begin{frame}{But It's a Forward Simulator}
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
			\begin{itemize}\itemsep10pt
				\item ${\sim}2000$ fixed parameters $\boldsymbol{\theta}$	
				\item One prediction out 
				\item This is the norm across the field
			\end{itemize}
		\end{column}
		\begin{column}{0.42\textwidth}
			\centering
			\begin{tikzpicture}[scale=0.75]
				...
				% No UQ
				\node[font=\scriptsize\itshape, text=red!70!black, anchor=west] at (1.5,-2.4) {No uncertainty};
			\end{tikzpicture}
		\end{column}
	\end{columns}

%	\vfill
%	MC4D predicts a \textbf{110-minute} cell cycle for syn3A. Experiment: ${\sim}$120 minutes. Close enough?
%
%	\medskip
%	\centering
%	\alert{But is that $110 \pm 5$ or $110 \pm 200$? The framework cannot answer this question.}
\end{frame}

% ----------------------------------------------------------------
\begin{frame}{The Inference Gap}
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
		...
		\begin{column}{0.42\textwidth}
			...
			\begin{tikzpicture}[scale=0.65]
				% Parameter "known" as a point
				\node[font=\small\bfseries, anchor=south] at (2,4.2) {What we pretend};
				\draw[->] (0,3.6) -- (4,3.6);
				\draw[very thick, red!70!black] (2,3.6) -- (2,3.75);
				\fill[red!70!black] (2,3.75) circle (3pt);
				\node[font=\scriptsize] at (2,3.3) {$k_{tx} = 0.05$};

				% Parameter reality: uncertain
				\node[font=\small\bfseries, anchor=south] at (2,1.6) {What we actually know};
				\draw[->] (0,0) -- (4,0);
				\draw[->] (0,0) -- (0,1.4);
				\fill[orange!25] plot[smooth, domain=0.3:3.7] (\x, {1.2*exp(-0.8*(\x-2)^2)}) -- (3.7,0) -- (0.3,0) -- cycle;
				\draw[thick, orange!70!black] plot[smooth, domain=0.3:3.7] (\x, {1.2*exp(-0.8*(\x-2)^2)});
				\node[font=\scriptsize] at (2,-0.3) {$k_{tx}$};
				\draw[<->, thick, orange!70!black] (0.9,-0.6) -- (3.1,-0.6);
				\node[font=\scriptsize, orange!70!black] at (2,-0.9) {plausible range};
			\end{tikzpicture}
		\end{column}
	\end{columns}
\end{frame}
...
% ----------------------------------------------------------------
\begin{frame}{The Standard Workflow}
	\centering
	\begin{tikzpicture}[
		scale=0.85,
	...
	\begin{itemize}\itemsep3pt
		...
		\item Uncertainty discarded at every arrow
...
\begin{frame}[plain]
	\centering
	\vfill
	{\Large\bfseries InferCell}\\[10pt]
	{\large\color{gray} An inference-first approach}
	\vfill
\end{frame}
...
% ----------------------------------------------------------------
% DRAFT v2 — compare with slide above, then delete one
\begin{frame}{What Inference Enables}
...
\begin{frame}[plain]
	\centering
	\vfill
	{\Large\bfseries How It Works}\\[10pt]
	{\large\color{gray} Architecture and inference graph}
	\vfill
\end{frame}

% ----------------------------------------------------------------
\begin{frame}{One Graph, End to End}
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
			\textbf{Probabilistic prediction requires gradients end-to-end.}
			\begin{itemize}\itemsep6pt
				\item MC4D: Python $+$ Cython $+$ CUDA $+$ LAMMPS --- no gradient path through language boundaries
				\item InferCell: solvers, models, and inference share \textbf{one compiled computational graph} in Julia
			\end{itemize}
		\end{column}
		\begin{column}{0.42\textwidth}
			\textbf{Julia ecosystem:}
			\begin{itemize}\itemsep5pt
				...
				\item Native interop between all of these
			\end{itemize}
		\end{column}
	\end{columns}
	...
	\vfill
	\centering
	\alert{AD, likelihood evaluation, and MCMC sampling fall out of this design.}
\end{frame}
...
% ----------------------------------------------------------------
\begin{frame}{Why Hybrid Inference?}
...
% ================================================================
% SECTION 3b: Technical Details (skippable)
% ================================================================
...
\begin{frame}[plain]
	\centering
	\vfill
	{\Large\bfseries Technical Details}\\[10pt]
	{\large\color{gray} Module interface and parameter specification}
	\vfill
\end{frame}

% ----------------------------------------------------------------
\begin{frame}{The Interface Is the Architecture}
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
		...
		\begin{column}{0.42\textwidth}
			...
			\begin{tikzpicture}[
				...
				% --- Two modules side by side at top ---
				...
				% Arrow from interface to inference
...
% ----------------------------------------------------------------
\begin{frame}[fragile]{Modules Declare Their Inference Mode}
...
% ----------------------------------------------------------------
\begin{frame}[fragile]{Parameters Carry Priors}
...
% ----------------------------------------------------------------
\begin{frame}[fragile]{End-to-End Workflow}
	\begin{lstlisting}[style=julia]
using InferCell
...
\begin{frame}[plain]
	\centering
	\vfill
	{\Large\bfseries In Practice}\\[10pt]
	{\large\color{gray} v1 scope and preliminary results}
	\vfill
\end{frame}


  \begin{frame}{v1 Scope: Three Modules}
	\centering
	\begin{tikzpicture}[
		scale=0.75,
		...
		% Block grouping
		\node[draw, dashed, thick, blue!50, rounded corners=8pt, fit=(txl)(met), inner sep=12pt, label={[font=\scriptsize\itshape, text=blue!60!black]above:DifferentiableBlock}] (diffblock) {};
		\node[draw, dashed, thick, orange!50, rounded corners=8pt, fit=(ssa), inner sep=12pt, label={[font=\scriptsize\itshape, text=orange!60!black]above:SimulationBlock}] (simblock) {};
...
% ----------------------------------------------------------------
\begin{frame}{Parameter Recovery: Joint ODE Inference (TX/TL + Metabolism)}
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
			\centering
			\includegraphics[width=0.85\textwidth]{figures/step3_corner.png}
		\end{column}
		\begin{column}{0.42\textwidth}
			\textbf{Joint posterior over 9 parameters}

			\bigskip
			\begin{itemize}\itemsep6pt
				\item TX/TL and Metabolism modules inferred jointly via NUTS
				\item Yellow crosshairs: ground truth values
				\item Posteriors recover the true parameters
				\item Off-diagonal panels reveal parameter correlations
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
% ----------------------------------------------------------------
\begin{frame}{Preliminary Results: SSA Module}
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
			\centering
			\includegraphics[width=0.85\textwidth]{figures/step3_ssa_posteriors.png}
		\end{column}
		\begin{column}{0.42\textwidth}
			\textbf{Parameter recovery: gene expression (SSA)}

			\bigskip
			\begin{itemize}\itemsep6pt
				\item 4 parameters inferred via ABC-SMC
				\item Green: unconditioned posteriors
				\item Blue: conditioned on shared parameters from the ODE module
				\item Conditioning tightens the posteriors. Information flows between modules
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Summary}
	\begin{enumerate}\itemsep14pt
		...
		\item \textbf{Joint inference over the coupled model.} Modules are composed into a hybrid inference graph  with tuneable coupling from cut posteriors to exact joint inference.
	\end{enumerate}
\end{frame}
...
\begin{frame}{Takeaways}
	...
	\begin{enumerate}\itemsep20pt
		...
		\item Interested? Get involved!
			...
			\item Workshop session at QUT in May (? TBC)
		\end{itemize}
	\end{enumerate}
...
% ----------------------------------------------------------------
\begin{frame}[noframenumbering]{TX/TL Module: Equations}
	\small
	Two-state ODE with four rate parameters:
	\begin{align*}
		\frac{d[\text{mRNA}]}{dt} &= k_{tx} - \gamma_{mRNA} \cdot [\text{mRNA}] \\[8pt]
	...
	\begin{tabular}{lll}
		...
		\midrule
		$k_{tx}$ & Transcription rate & $\text{LogNormal}(-3.0,\; 0.5)$ \\
		$k_{tl}$ & Translation rate & $\text{LogNormal}(-1.5,\; 0.5)$ \\
		$\gamma_{mRNA}$ & mRNA degradation rate & $\text{LogNormal}(-4.0,\; 0.5)$ \\
		$\gamma_{prot}$ & Protein degradation rate & $\text{LogNormal}(-5.0,\; 0.5)$ \\
		$\sigma$ & Observation noise & $\text{HalfNormal}(0.5)$ \\
		\bottomrule
	\end{tabular}

	\bigskip
	{\small Observation noise $\sigma$ is inferred, not fixed.}
\end{frame}


\begin{frame}[noframenumbering]{Complementary to Existing Work}
	\centering
	\begin{tikzpicture}[
		>=Stealth,
		axis/.style={thick, ->},
		pt/.style={draw, thick, rounded corners=4pt, minimum height=0.8cm, minimum width=2.8cm, align=center, font=\small},
		]
		% Axes
		\draw[axis] (0,0) -- (10,0) node[right, font=\small] {Spatial resolution};
		\draw[axis] (0,0) -- (0,5.5) node[above, font=\small] {Inference capability};

		% Projects
		\node[pt, fill=gray!15] (karr) at (1.5,0.8) {Karr 2012};
		\node[pt, fill=gray!15] (covert) at (5,1.2) {Macklin 2020};
		\node[pt, fill=blue!15] (mc4d) at (8,1.5) {MC4D (2026)};
		\node[pt, fill=green!20] (ic) at (2.5,4.2) {\textbf{InferCell}};

		% Annotations
		\node[font=\scriptsize\itshape, text=gray, anchor=west] at (5.5,0.6) {forward simulation};
		\node[font=\scriptsize\itshape, text=blue!70!black, anchor=west] at (8.5,2.2) {spatial, forward-only};

		% Convergence arrow
		\draw[->, dashed, thick, red!50!black] (ic.east) -- (7,3.5)
		node[midway, above, font=\scriptsize, sloped] {future convergence};
		\draw[->, dashed, thick, red!50!black] (mc4d.north) -- (7,3.5);
		\node[font=\scriptsize, text=red!50!black] at (7.8,4.1) {spatial + inference};
	\end{tikzpicture}
\end{frame}

% --- CRN structural inference ---
