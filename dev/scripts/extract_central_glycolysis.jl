...
#
# The width column keeps its upstream name, `!UnconstrainedGeometricStd`,
# rather than being emitted as a bare `!GeometricStd`. There is no balanced
# width in the source: the location comes from the balanced `!Mode` and the
# spread from the unconstrained fit, and renaming it would hide exactly the
# mode-versus-mean confusion the two columns above exist to warn about. For
# every concentration and every Michaelis constant here the two agree exactly,
# so the pairing costs nothing; for ten of the twenty catalytic constants they
# do not, and spec §4 D1 carries the measurement.
#
# Usage:
...
# The upstream width column, carried through under its own name.
const GSTD_COLUMN = "!UnconstrainedGeometricStd"
...
The class-count assertion below catches a table that changed *shape*; it cannot
catch one that changed a value, and a regeneration against a different revision
would then round-trip cleanly against the wrong upstream. Spec §5's guarantee
that nothing is irreplaceable holds only while the commit reference stays
correct, so the reference is checked rather than recorded. A checkout that is
not a git repository is warned about rather than refused — a tarball is a
legitimate way to have the source.
"""
function check_commit(checkout::AbstractString)
    ...
    head == UPSTREAM_COMMIT || error(
        "$checkout is at $head, not the $UPSTREAM_COMMIT this extract is derived " *
        "from. Check the revision out, or update UPSTREAM_COMMIT here and in " *
        "src/organisms/coreA/data/README.md and regenerate deliberately.")
    return nothing
end
...
function main()
    ...
    c_gstd = column(header, GSTD_COLUMN)
    ...
    open(out, "w") do io
        ...
        println(io, "!ID\t!Mode\t$(GSTD_COLUMN)\t!UpstreamRow")
    ...
    println("sha256: ", bytes2hex(sha256(read(out))))
end
