...
# ---- the single-gene TranscriptionTranslation, the model the byte-identity fixture pins ----
# Its own dynamics branches on gene_names at run time and is not inferable, so
# the composed RHS is expected to allocate on both paths; this measures by how
# much rather than asserting it.
txl = [TranscriptionTranslation()]
prob_txl = build_problem(txl; tspan = (0.0, 50.0))
ctx_txl = InferCell._build_contexts(txl); InferCell._resolve_coupling(txl, ctx_txl)
rhs_txl_legacy = legacy_build_rhs(txl, ctx_txl)
rhs_txl_static = prob_txl.f.f
alloc_oop(rhs_txl_legacy, prob_txl.u0, prob_txl.p, 0.0); alloc_oop(rhs_txl_static, prob_txl.u0, prob_txl.p, 0.0)
time_oop(rhs_txl_legacy, prob_txl.u0, prob_txl.p, 0.0, 10); time_oop(rhs_txl_static, prob_txl.u0, prob_txl.p, 0.0, 10)
txl_rows = [
    ("TranscriptionTranslation, legacy", alloc_oop(rhs_txl_legacy, prob_txl.u0, prob_txl.p, 0.0),
     time_oop(rhs_txl_legacy, prob_txl.u0, prob_txl.p, 0.0, NCALLS)[1]),
    ("TranscriptionTranslation, phase-1 static", alloc_oop(rhs_txl_static, prob_txl.u0, prob_txl.p, 0.0),
     time_oop(rhs_txl_static, prob_txl.u0, prob_txl.p, 0.0, NCALLS)[1]),
]

commit = strip(read(`git rev-parse --short HEAD`, String))
dirty = !isempty(strip(read(`git status --porcelain -- src test Project.toml Manifest.toml dev/scripts/bench_rhs_containers.jl`, String)))
jobid = get(ENV, "SLURM_JOB_ID", "not under Slurm")

io = IOBuffer()
println(io, "# RHS container benchmark at 32 states (spec task 1.6)")
println(io)
println(io, "Generated by `dev/scripts/bench_rhs_containers.jl` at commit $commit", dirty ? " (WARNING: tree dirty)" : "",
...
println(io)
println(io)
println(io, "Single-gene `TranscriptionTranslation` (2 states, no contributions), the model the byte-identity fixture pins:")
println(io)
println(io, "| RHS | bytes / call | ns / call |")
println(io, "|---|---|---|")
for (name, b, ns) in txl_rows
    @printf(io, "| %s | %d | %.0f |\n", name, b, ns)
end
println(io)
println(io, "Its own `dynamics` branches on `gene_names` at run time and is not inferable, so the framework's ")
println(io, "zero-allocation path cannot make the composed function allocation-free; the rows show what the ")
println(io, "framework change alone saved for it.")
println(io)
