...
input has an inbound edge, and every inbound mass edge on a state the module
does not integrate is listed in `inputs` — so a typed declaration that drifts
out of step with `inputs` is caught rather than silently disagreeing. Inputs
naming non-registry state (the legacy blocks' mRNA and protein) are outside
the typed contract and pass through unchecked.
"""
