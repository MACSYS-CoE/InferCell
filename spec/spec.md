...
- [x] 7.1 Vendor two extracts — eleven rate constants with no uncertainty column
  ...
  **Done:** `pts_transport.tsv` (11 rows, no `GeometricStd` column) and
  ...
  asserted-prior enumeration returns 22 for this module — the 11 constants, the
  radius, the volume ratio and the 9 initial conditions — and a balanced
  glycolytic parameter composed beside it does not appear. `ambiguity_report`
  ...
  **One divergence from the archived design.** Its D3 table used a rounded
  ...
  totals land on 353, 314, 290 and 831 exactly rather than on 353.007.
...
- [x] 7.4 Integrate external lactate with the volume ratio (D6) — verify by
  ...
  **1.46836 mM** and external reaches **0.006877 mM** after 6,300 s, which is
  **0.468%** — D6's table says 0.47%, and a ratio of 1e4 misses the criterion,
  as its 4.7% row says. The settling value is `production/0.075` **plus** the
  ...
  **The whole 6,300 s testset costs 19.5 s**, so it runs in the default suite
  rather than behind a flag — which answers §9's open question, "whether the
  full-cycle checks belong in the default test suite", on the measured number
  for this check.
...
- [x] 7.5 Declare the boundary, including external glucose clamped at 40 mM with
  ...
  **Ten edges, not the archived design's five, and the reason is worth
  carrying.** Both reversible steps read the pool their forward term fills, and
  `inputs` — the only channel that wires a foreign state into `dynamics` — is
  held to the *inbound* edges. So `M_pyr_c` and `M_g6p_c` each carry an edge in
  both directions, and `contributed_states` returns one net signed term for
  each. Phase 1 is why: before it, the archived design could declare production
  and leave it unexecuted.
...
- [x] 7.7 Check carrier conservation as four separate assertions — verify by four
  ...
  *linear* invariant of a right-hand side whose two terms are exact IEEE
  ...
  `(1e-10, 1e-8)`, and unchanged at `(1e-11, 1e-9)` — the ptsI one rose, to
  9.02e-17. §3's fivefold clause is therefore unsatisfiable for this class, and
  would have been for check 3 too. The principle now names two classes and this
