...
\begin{frame}{Level-2 Iterative Boundary: Posterior Tightening}
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
			\centering
			\includegraphics[width=0.95\textwidth]{figures/step4_corner_talk_shared.png}\\[3pt]
			{\scriptsize\itshape Single-pass (blue) vs Level-2 iterative (orange) posteriors on the shared params; truth in black.}
		\end{column}
		\begin{column}{0.42\textwidth}
			\footnotesize
			...
			\begin{itemize}\itemsep3pt
				...
				\item SSA block: \textbf{bursty} (telegraph) regulator $-$ $k_{tx} \to k_{tx,\mathrm{burst}}$
				\item Shared params: $k_{tl},\;\gamma_{mRNA},\;\gamma_{prot}$ (3, vs 4 for the constitutive SSA on the previous slide)
			\end{itemize}
