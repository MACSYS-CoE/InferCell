...
\begin{frame}{But the parameters themselves are uncertain (epistemic uncertainty)}
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
			\vspace*{2.5em}
			\begin{enumerate}
				\item Parameters are uncertain.
				\item Uncertainty propagates nonlinearly.\\
				{\small\itshape Uncertain rates $\to$ mRNA $\to$ protein $\to$ flux $\to$ growth rate.}
				\item The architecture has no place to store/propagate uncertainty
			\end{enumerate}
		\end{column}
		\begin{column}{0.42\textwidth}
			\centering
	...
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
			\begin{itemize}\itemsep7pt
				...
				{\small One language, one computational graph}
			\end{itemize}
		\end{column}
		\begin{column}{0.42\textwidth}
			\centering
	...
	\begin{columns}[T]
		...
		\begin{column}{0.48\textwidth}
			...
			\begin{itemize}\itemsep4pt
				...
				\item[] {\scriptsize\itshape ``Is doubling time 110\,$\pm$\,5\,min, or 60--200?''}
				\item Parameter calibration
				\item[] {\scriptsize\itshape ``What is $k_{tx}$ for this gene, given our data?''}
				\item Identifiability
				\item[] {\scriptsize\itshape ``Does data constrain $k_{tx}$ alone, or only $k_{tx}/\gamma$?''}
			\end{itemize}
		...
		\begin{column}{0.48\textwidth}
			\begin{itemize}\itemsep4pt
				...
				\item[] {\scriptsize\itshape ``Does the data prefer ODE or SSA dynamics?''}
				\item Experimental design
				\item[] {\scriptsize\itshape ``Which experiment most reduces uncertainty in $\gamma_{prot}$?''}
				\item Fundamental biology
				\item[] {\scriptsize\itshape ``Does the data require enzyme feedback in metabolism?''}
			\end{itemize}
	...
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
			...
			\begin{itemize}\itemsep5pt
				...
				\item Module boundaries require re-encoding data (file IO, language bindings, GPU---CPU copy) rather than passing pointers
				\item Gradients don't compose across language boundaries
			\end{itemize}
			
						\textbf{ InferCell: one Julia stack}
		...
		\begin{column}{0.42\textwidth}
			...
			\begin{tikzpicture}[
				...
				\draw[<->, thick, colWarnLine] (bd) -- node[iolabel, below, pos=0.4, yshift=8pt, xshift=18pt]
...
\begin{frame}{What Becomes Possible (Engineering)}
	...
	\begin{columns}[T]
		\begin{column}{0.48\textwidth}
			...
			\begin{itemize}\itemsep0pt\parskip0pt
				\item Whole model as one program --- one type system, one IR, one GPU pipeline
			\end{itemize}

			\medskip
			\textbf{End-to-end gradients}
			\begin{itemize}\itemsep0pt\parskip0pt
				\item AD through solvers, models, and inference --- HMC/NUTS, BOED, Fisher information 
			\end{itemize}

			\medskip
			\textbf{Hybrid inference}
			\begin{itemize}\itemsep0pt\parskip0pt
				\item Combine NUTS and SBI in one inference pipeline --- no inter-language plumbing
			\end{itemize}
		\end{column}
		\begin{column}{0.48\textwidth}
...
% ----------------------------------------------------------------
\begin{frame}{Joint parameter estimation - differentiable block}
	\centering
...
\begin{frame}{Joint parameter estimation - simulation block}
	\centering
\includegraphics[width=0.48\textwidth]{figures/step3_ssa_posteriors.png}
\end{frame}


\begin{frame}{Joint parameter estimation - differentiable block - Gibbs-like}
			\centering
			\vspace{3mm}
			\includegraphics[width=0.47\textwidth]{figures/step4_corner_talk_shared.png}
\end{frame}
...
% ----------------------------------------------------------------
\begin{frame}[noframenumbering]{What ``Crossed by Serialization'' Means}
	\footnotesize
	\textit{A reference pass shares memory; a serialization re-encodes the data on the other side. The polyglot stack does the second at every boundary.}

	\medskip
	\begin{columns}[T]
		\begin{column}{0.48\textwidth}
			\textbf{By reference (within one runtime)}
			\begin{itemize}\itemsep1pt\parskip0pt
				\item Pointer passed, not bytes
				\item Same memory, same type system, same garbage collector
				\item AD tape stays attached
				\item Distribution objects survive intact
				\item Cost: O(1), nanoseconds
			\end{itemize}
		\end{column}
		\begin{column}{0.48\textwidth}
			\textbf{By serialization (across runtimes)}
			\begin{itemize}\itemsep1pt\parskip0pt
				\item Bytes encoded, shipped, decoded
				\item Fresh allocation; type information re-established
				\item AD tape broken at the boundary
				\item Distributions reduced to point values
				\item Cost: O(N) per call
			\end{itemize}
		\end{column}
	\end{columns}

	\medskip
	\textbf{Four flavours in a typical polyglot WCM:}
	\begin{itemize}\itemsep1pt\parskip0pt
		\item \textbf{Subprocess (LAMMPS):} input file written, output file read --- disk IO
		\item \textbf{Language bindings (\texttt{lm} C++):} pybind11/ctypes marshal types in shared address space
		\item \textbf{GPU $\leftrightarrow$ CPU (CUDA RDME):} PCIe memory copy at every kernel boundary
		\item \textbf{C/Fortran wrappers (Python ODE):} ``in-process'' but NumPy $\leftrightarrow$ C arrays marshalled per call
	\end{itemize}
\end{frame}

% ----------------------------------------------------------------
\begin{frame}[noframenumbering]{What Dies at a Module Boundary}
	\footnotesize
	\textit{Serialization re-encodes data and strips off everything except raw values. Three different objects survive a reference pass but die at a serialization. The polyglot stack pays each cost; a unified runtime pays none.}
	...
	\begin{tabular}{p{3cm} p{4.8cm} p{4.8cm}}
		\toprule
		\textbf{Object that dies} & \textbf{What it costs to lose} & \textbf{What InferCell preserves} \\
		\midrule
		Probability distribution & Information about $\theta$ destroyed at every boundary (data processing inequality) & Full posterior carried across modules; uncertainty propagates by construction \\
		Derivative / AD tape     & No end-to-end gradients $\Rightarrow$ no NUTS, no HMC, no gradient-based BOED & End-to-end differentiability through solvers, models, inference \\
		Type information         & Units lost, no compile-time validation, brittle cross-module contracts & Shared Julia type system; module interface is a typed contract \\
		\bottomrule
	\end{tabular}

	\bigskip
	\raggedright
	\textbf{The unification.} One root cause: serialization is destructive. One architectural answer: keep the whole model in one runtime so boundaries are pointer passes. The talk's two ``roots'' --- probabilistic semantics and unified substrate --- are aspects of the same boundary-preservation property.
\end{frame}
...
% ================================================================
% ML surrogates: differentiability and the block-mover lever
% ================================================================

% ----------------------------------------------------------------
\begin{frame}[noframenumbering]{Neural Surrogates: Differentiable by Construction}
	\footnotesize
	\textit{An ML surrogate replaces an expensive module with a learned function. Because Julia's ML stack (Flux.jl, Lux.jl) shares the AD substrate, gradients flow through surrogates end-to-end --- when the surrogate lives in the same runtime.}

	\bigskip
	\begin{columns}[T]
		\begin{column}{0.48\textwidth}
			\textbf{Differentiable surrogate types}
			\begin{itemize}\itemsep3pt\parskip0pt
				\item Neural networks (MLP, neural ODE) --- Flux.jl, Lux.jl
				\item Normalising flows --- Bijectors.jl
				\item Gaussian processes --- AbstractGPs.jl
			\end{itemize}

			\bigskip
			\textbf{Non-differentiable surrogates}
			\begin{itemize}\itemsep3pt\parskip0pt
				\item Trees, argmax outputs, non-reparameterised samplers
				\item Slot into the SimulationBlock with SBI --- the hybrid architecture already handles this case
			\end{itemize}
		\end{column}
		\begin{column}{0.48\textwidth}
			\textbf{The catch --- same AD substrate}
			\begin{itemize}\itemsep3pt\parskip0pt
				\item Surrogates are only differentiable end-to-end if they live in the same AD universe as the rest of the model
				\item A PyTorch surrogate plus a Julia model is still a polyglot stack
				\item Gradients die at the language boundary, same as for any other serialised module
				\item Julia's ML ecosystem (Flux, Lux) integrates with Zygote/Enzyme transparently; PyTorch-via-PyCall does not
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
% ----------------------------------------------------------------
\begin{frame}[noframenumbering]{Surrogates Move Modules Between Blocks}
	\footnotesize
	\textit{The architectural lever: a differentiable surrogate trained on a stochastic simulator's outputs can move a module from SimulationBlock to DifferentiableBlock.}
	...
	\begin{columns}[T]
		\begin{column}{0.42\textwidth}
			...
			\begin{tikzpicture}[
				scale=0.8,
				mod/.style={draw, thick, rounded corners=4pt, minimum height=0.8cm, minimum width=3cm, align=center, font=\small},
				blk/.style={font=\footnotesize\itshape, anchor=west},
				>=Stealth
			]
				\node[mod, fill=orange!15] (ssa) at (0,3) {SSA module};
				\node[blk, text=orange!70!black] at (1.9,3) {SimulationBlock (SBI)};

				\draw[->, very thick, blue!60!black] (0,2.3) -- (0,1.0);
				\node[font=\scriptsize, anchor=west] at (0.2,1.65) {train surrogate};
				\node[font=\scriptsize, anchor=west] at (0.2,1.25) {offline};

				\node[mod, fill=blue!15] (surr) at (0,0.3) {Neural surrogate};
				\node[blk, text=blue!70!black] at (1.9,0.3) {DifferentiableBlock (NUTS)};
			\end{tikzpicture}
		\end{column}
		\begin{column}{0.55\textwidth}
			\textbf{Recipe}
			\begin{enumerate}\itemsep3pt\parskip0pt
				\item Run slow simulator offline; collect $(\theta, y)$ pairs
				\item Train a differentiable surrogate (e.g.\ normalising flow over outputs)
				\item Replace the simulator module with the surrogate
				\item Module migrates to the DifferentiableBlock; NUTS now handles it
			\end{enumerate}

			\medskip
			\textbf{Payoff}
			\begin{itemize}\itemsep3pt\parskip0pt
				\item Orders-of-magnitude speedup at inference time
				\item End-to-end gradients across a previously stochastic module
				\item Inference graph adapts to module changes --- not static
			\end{itemize}
		\end{column}
	\end{columns}
	...
	\bigskip
	\centering
	{\scriptsize\textbf{Related literature:} neural likelihood/posterior estimation in SBI (Cranmer, Brehmer, Louppe \emph{et al.}). The architecture lets you mix and match these tools at the module level.}
\end{frame}
