...
InferCell exports its public API across nineteen source files. The pages in this section are organised by source module; most pages list the public API with the same docstring text returned by `?<symbol>` in the Julia REPL, while the Core A′ page documents its contract as prose.
