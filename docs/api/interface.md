...
`contributed_states` and `contributions` are the outbound counterpart of `inputs` and `dynamics`: where `inputs` lets a module *read* a state another module integrates, `contributions` lets it *write* a derivative term into one. The orchestrator adds the term to the owner's derivative at the owner's global index; a module never writes outside its own slice directly. The species must be Core A′ registry names, may not be chemostatted, and must be owned by some module in the composition. Where a module declares any coupling edge, every mass or currency edge (either direction) on a dynamic registry species it does not own must have a matching contribution, and every contribution a matching edge — the resolver checks both directions, as it does for `inputs`. A module that consumes a foreign pool contributes a negative term; one that produces into it contributes a positive one.
