...
| `VolumeEdge` | outbound: counts set surface area, hence volume, hence every concentration. Inbound: the geometry re-enters an ODE rate law | inbound only: `param_slot`, `quantity` (the flagged states come from `membrane_protein_states`) |
