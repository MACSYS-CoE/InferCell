...
Each sub-model declares its inputs via [`inputs`](@ref). The orchestrator resolves these against the global state ownership table — every declared input must be a [`states`](@ref) entry of some sub-model in the composition — and threads the resolved values into `dynamics(u, p, t, m, u_inputs)`. Models with no inputs use the simpler `dynamics(u, p, t, m)` overload.
